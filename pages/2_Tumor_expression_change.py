...
genes = st.text_input('Enter genes of interest (separated by commas without spaces):').upper().strip(' ')
correct_genes = []
# Identify if indicated gene is present in the data
data = pd.read_csv('Data/log2FC_expression.csv')
if genes == '':
    st.error('Introduce at least one gene')
else:
    if ' ' in genes:
...
if st.button('Show Fold Change'):
    ...
    if genes == '':
        ...
        st.error('Please check if the intorduce gene is found or if data genes are introduced separated by commas without spaces as in the following example: **EGFR,FGFR1,CD19**')