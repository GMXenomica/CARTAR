...
genes = st.text_input('Enter gene symbols of interest (separated by commas without spaces):').upper().strip(' ')
correct_genes = []
...
if st.button('Show Fold Change'):
    ...
    if genes == '':
        ...
        st.error('Please check if the intorduce gene symbol is found or if gene symbols are introduced separated by commas without spaces as in the following example: **EGFR,FGFR1,CD19**')