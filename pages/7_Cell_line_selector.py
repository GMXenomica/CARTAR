...
st.write(
    'This tool allows the identification of cancer cell lines from the Cancer Cell Line Encyclopedia (CCLE) with expression of the inidicate gene highe or lower than a stablished threshold according to your interests. You can indicate the cancer cell line lienage you are intersted in. The cancer cell line expression data is obtained form [DepMap Public 23Q2 Primary Files](https://depmap.org/portal/download/all/). This tool will return an interactive barplot showing expression of the gene in those cell lines by lineage that meet the stablished threshold and a table with all the cell lines relevant data.'
)
...
url = 'https://raw.githubusercontent.com/mighg/CARTAR/main/genes_cells.csv'
...
gene = st.text_input('Introduce gene symbol').upper()  # Introduce gene
if gene == '':
...
tumors = st.multiselect('Choose tumoral tissue', tissue_options) # Introduce tumor tissues or none if you want all tumors
if not tumors:
    #tumors = tissue_options
    st.success('If not tumoral tissue is seleceted all will be shown')
expression =  st.radio('Choose underexpression or overexpression', expression_options)# Introduce under to look for cell with expression values below the threshold or over for expression values aboce the threshold
value =  st.text_input('Introduce threshold expression value:') # Introduce threshold value
scale =  st.radio('Choose scale', scale_options) # Introduce scale 
if value:
    if ',' in value:
        value = value.replace(',','.')
    value = float(value)
...
if st.button('Find cell lines'):
    ...
    if table_data.size > 0:
        ...
        st.write(
            f'All relevant data of the selected cell lines is shown in the table below, including lineage, primary diseases and disease subtype. You can click in the column names to order the genes according to that column from higher to lower or viceversa. By clicking in a cell you can see the value with all the decimals.'
        )
