...
if st.button('Show tumor-associated genes'):
    ...
    if not selected_genes:
        ...
        st.error(f'Fold Change (FC) used is beyond the valid range. The maximum allowable value in this tumor is {max_FC}.')
    else:
    ...
    if table.size > 0:
        ...
        st.write(
            f'The table below presents all pertinent data, encompassing the log2(FC) between "Primary {tumor} tumor" and "Control" samples, along with the p-value for each gene surpassing the specified threshold. You can enhance exploration by clicking on the column names to arrange genes based on that column, either from higher to lower or vice versa. Please note that **p-values under 0.001 are rounded to 0**; for the complete decimal value, click on the respective cell.'
        )
