...
if st.button('Show tumor-associated genes'):
    ...
    if table.size > 0:
        ...
        text = f'Interactive volcano plot showing the log2(FC) between \"Primary {tumor} tumor\" and \"Control\" samples and the p-value for all genes. Significant genes (p-value < 0.05) with a FC higher than the specified threshold are shown in <span style="color:{color1}">red, while the rest of genes are shown in <span style="color:{color1}">grey. The name of the gene is displayed by hovering the cursor over the plot</span>.'
