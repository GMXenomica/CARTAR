...
if st.button('Show tumor-associated genes'):
    ...
    if table.size > 0:
        ...
        st.write(
            f'Interactive volcano plot showing the log2(FC) between \"Primary {tumor} tumor\" and \"Control\" samples and the p-value for all genes. Significant genes (p-value < 0.05) with a FC higher than the specified threshold are shown in :red[**rgba(179, 48, 41, 0.89)**], while the rest of genes are shown in :grey[**rgba(156, 165, 196, 0.95)**]. The name of the gene is displayed by hovering the cursor over the plot.'
        )
