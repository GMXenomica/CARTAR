...
if st.button('Create barplot'):
    if gene != '': 
        ...
        metastatic = [] # List with median values of metastatic groups (NA if not available)
        primary = [] # List with median values of primary tumor groups
        normal = [] # List with median values of normal groups
        url = 'https://gitlab.com/gmx2/CARTAR/-/raw/main/targetable_gene_Tpm_TumorVsControl_final.csv' # Read expression data for the samples
        response = requests.get(url, stream=True)
        m_indexes = [] # List with indexes of metastatic groups
        p_indexes = [] # List with indexes values of primary tumor groups
        n_indexes = [] # List with indexes values of normal groups
        firstline = 1
        for line in response.iter_lines(decode_unicode=True):
            fields = line.split(',')
            # Identify position of fields belonging to each group of the selected tumors in the firstline of the file
            if firstline: 
                n = 0
                ...
                # Identify median value for each group in the selected tumors and selected scale, and add it to corresponding list
                if gene == fields[0]:
                    if scale == 'log2(TPM+1)':
        ...
        # Creation of the dictionary containing all needed information for the plot
        data = {'Tumor type': [], 'Value': [], 'Group': [], 'Label': []}
        for j, category in enumerate(categories):
            for i, condition in enumerate(classes):
                data['Tumor type'].append(category)
                data['Group'].append(condition)
        ...
        fig = px.bar(df, x='Tumor type', y='Value', color='Group', color_discrete_sequence=colors,custom_data=['Label'])
        fig.update_traces(hovertemplate='%{customdata}')
        # Customize graph titile and axis names
        if scale == 'log2(TPM+1)':
            fig.update_layout(title=f'{gene} expression comparison between tumoral conditions', title_x=0.25, xaxis_title='Tumor type', yaxis_title= f'{gene} expression in log2(TPM+1)', barmode='group')
        else:
            fig.update_layout(title=f'{gene} expression comparison between tumoral conditions', title_x=0.25, xaxis_title='Tumor type', yaxis_title= f'{gene} expression in TPM', barmode='group')
        # Show the graph and table
        st.header('Interactive barplot', divider='rainbow')
        st.plotly_chart(fig,use_container_width=True)
        st.write(
            f'The above figure shows an interactive barplot for {gene} expression in {scale} across \"Metastatic\", \"Primary tumor\" and \"Control\" samples of selected tumors to compare the expression between theses groups. No statistical significance is shown. If you are interested in knowing the statistical significance beteween \"Primary tumor\" and \"Control\" samples you must go to [**Tumor gene expression tool**](http://localhost:8501/Tumor_gene_expression).'
        )
        st.header('Data table', divider='rainbow')
        st.write(
            f'Median of each group for the selected tumors are shown in the table below. If you are interested in knowing the log2(Fold Change) between \"Primary tumor\" and \"Control\" samples, as well as the statistical significance, you must go to [**Tumor gene expression tool**](http://localhost:8501/Tumor_gene_expression). You can click in the column names to order the tumors according to that column from higher to lower or viceversa. By clicking in a cell you can see the value with all the decimals.'
        )
