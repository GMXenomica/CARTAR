...
st.markdown('- ___Tumor-associated antigens:___ identify tumor-associated antigens')
st.markdown('- ___Tumor expression change:___ explore fold change expression values of a gene or set of genes accross specified tumors')
st.markdown('- ___Tumor median expression:___ visualize median expression values of a candidate gene across primary tumors, metastatic (if available), and control samples')
st.markdown('- ___Tumor gene expression:___ analyze expression values of a specific gene across primary tumor and control samples of specified tumors')
st.markdown('- ___Tissue gene expression:___ study off-tumor gene expression to assess specificity of candidate target genes')
st.markdown('- ___Metastatic gene expression:___ analyze expression values of a specific gene across primary tumor, metastatic and control samples on Skin Cutaneous Melanoma (SKCM)')
st.markdown('- ___Logic-gated CAR:___ explore the correlation between the expression values of two genes in primary tumor and control samples of a specified tumor for the design of dual- targeting CAR therapy')
st.markdown('- ___Cell line selector:___ identify cancer cell lines with desired expression values of target gene')
